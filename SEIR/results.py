...
class Results():
    ...
    def plot_quick_summary(self, comp = 'cumI'):
        ...
        for nd in range(self.s.nnodes):
            expand_data[nd] = {}
            expand_data[nd][comp] = pd.DataFrame(index = pd.date_range(self.ti, self.tf, freq = self.freq), 
        ...
        q95 = pd.DataFrame(index = pd.date_range(self.ti, self.tf, freq = self.freq), columns = np.arange(self.s.nnodes))
        for i, nd in enumerate(np.arange(self.s.nnodes)):
    ...
    def build_comp_data(self):
        ...
        for nd in range(self.s.nnodes):
            expand_data[nd] = {}
            expand_data[nd]['S'] = pd.DataFrame(index = pd.date_range(self.ti, self.tf, freq = self.freq), 
                                                    columns = np.arange(nsim))
            expand_data[nd]['E'] = pd.DataFrame(index = pd.date_range(self.ti, self.tf, freq = self.freq), 
                                                    columns = np.arange(nsim))
            expand_data[nd]['I1'] = pd.DataFrame(index = pd.date_range(self.ti, self.tf, freq = self.freq), 
                                                    columns = np.arange(nsim))
            expand_data[nd]['I2'] = pd.DataFrame(index = pd.date_range(self.ti, self.tf, freq = self.freq), 
                                                    columns = np.arange(nsim))
            expand_data[nd]['I3'] = pd.DataFrame(index = pd.date_range(self.ti, self.tf, freq = self.freq), 
                                                    columns = np.arange(nsim))
            expand_data[nd]['R'] = pd.DataFrame(index = pd.date_range(self.ti, self.tf, freq = self.freq), 
                                                    columns = np.arange(nsim))
            expand_data[nd]['cumI'] = pd.DataFrame(index = pd.date_range(self.ti, self.tf, freq = self.freq), 
                                                    columns = np.arange(nsim))
            for sim in range(self.s.nsim):
    ...
    def save_output_for_R(self, seir):
        ...
        for s in range(self.s.nsim):
            ...
            out_df = pd.DataFrame(out_arr, columns = ['comp'] + list(self.s.spatset.data['geoid'].astype(int)),
                                index = pd.date_range(self.ti, self.tf, freq='D').repeat(ncomp+1))
    ...
    def plot_all_comp(self, nodes):
        ...
        for c, comp in enumerate(['S', 'E', 'I1', 'I2', 'I3', 'R', 'cumI']):
            ...
            ax.set_title(comp)
            for i, nd in enumerate(nodes):
                ax.plot(self.comp_data[nd][comp].quantile(.5, axis = 1), c = self.colors[i%4])
                ax.fill_between(self.comp_data[nd][comp].index, self.comp_data[nd][comp].quantile(.05, axis = 1), 
                                  self.comp_data[nd][comp].quantile(.95, axis = 1), alpha =.3, 
                                  facecolor = self.colors[i%4])
            
        ax.legend([self.s.spatset.data['geoid'][i] for i in nodes])
        fig.autofmt_xdate()
        if not self.s.interactive:
