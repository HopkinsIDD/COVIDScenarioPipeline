...
We provide functions to access results and help produce reports in the [report.generation](https://github.com/HopkinsIDD/COVIDScenarioPipeline/tree/dataseed/R/pkgs/report.generation) R package in `COVIDScenarioPipeline`.
...
```{r, eval = FALSE}
rmarkdown::draft("Maryland_Report.Rmd", template = "state_report", package = "report.generation")
```
...
## Actual Steps I went through to create a minimal working repository (by commit)
 ...
 - Install all relevant R packages from github:
   ...
   devtools::install_github(repo='HopkinsIDD/COVIDScenarioPipeline', ref='dataseed', subdir='R/pkgs/report.generation')
   ```
