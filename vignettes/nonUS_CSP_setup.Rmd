...
The config file config.yml controls all of the options currently available. (See  [this page for more details](https://github.com/HopkinsIDD/COVID19_Minimal/wiki/Config-File-Options).) 

An example config file has been included int he COVID19_Minimal repository. 
Start by replacing the US version with the non-US version using the following commands:

```
rm config.yml # remove US-specific config
mv config_nonUS.yml config.yml # copy non-US version to the generic file name
```
...
| spatial_setup::modeled_states | This should be a list of the countries you want to simulate, with each country on it's own line preceded by `-` | modeled_states:<br>   - WES |
...
These data are automatically generated by the `create_seeding.R` script through running the following command:     

`Rscript <path-to-COVIDScenarioPipeline>/COVIDScenarioPipeline/R/scripts/create_seeding.R -c config.yml -d <path-to-case-data>`
    
This should look like this, since you should be in the home directory of your repository:
`Rscript ./COVIDScenarioPipeline/R/scripts/create_seeding.R -c config.yml -d data/case_data/case_data.csv`    
