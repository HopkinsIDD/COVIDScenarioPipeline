...
***--> MODIFY COVID19_Minimal FOR THIS <--***    
...
If you want to install all the packages on your local computer, in COVIDScenarioPipline, see the R requirements in packages.R and local_install.R, OS requirements in Dockerfile, and Python requirements in requirements.txt. Good luck.
...
spatial_setup:
  ...
  - WES
  - ESS
  geoid_len: 5            # Since the user will create the `geoid` variable, length needs to be specified
  ...
```
...
|-------------|----------------------|---------|
| name        | Give it a name       | westeros_essos  |
...
In non-US CSP applications, setup data are not incorporated into the repositories so must be incorportated manually. For the user, this has been consolidated into a single script to format inputted data correctly for CSP use.
...
This shapefile must have a GEOID column and be somewhere in the `data` directory. This is now done in the `setup_country_data.R` script.




## Calculate Age-specific outcomes parameters

To use age-specific outcomes adjusted for the age distributions with the nodes, users can use the [*covidSeverity*](https://github.com/HopkinsIDD/covidSeverity) R package. See the `age-adjustment-example.Rmd` in the vignettes for a detailed tutorial. This is all done automatically in the `setup_country_data.R` script.


### Using WorldPop

Using the `covidSeverity` package we can directly download and generate district-specific age distibutions of any country of interest.
Users only need to provide their own shapefiles for this.


#### 1. Get the age distributions by admin level 2 from WorldPop
...
filenames <- download_worldpop_agetifs(country="ZMB", year="2020", save_dir="../data", cores=4)

# Combine data and consolidate to districts
age_pop_data <- load_worldpop_age(shp="data/ZMB/bgd_admbnda_adm2_bbs_20180410/bgd_admbnda_adm2_bbs_20180410.shp", 
                                  country="ZMB", year="2020", save_dir="data", cores=4)
# Convert to 10-year age groups
age_pop_10yr <- convert_wp_10yr(age_pop_data)
readr::write_csv(age_pop_10yr, "data/ZMB/ZMB_age_pop_10yr.csv")
readr::write_csv(age_pop_data, "data/ZMB/ZMB_age_pop_data.csv")
...
# Save a list of names and geoids for matching
...
This should have three columns to provide to a poisson distribution to determine the actual number of imported/seeded cases 
...
1. Files for mobility data and population data for the nodes of the admin level 1 or 2 spatial areas in the directory `data/geodata`. These should both be .csv files, and GEOIDs must be consistent between them.
    ...
    b. *Population data*:    
        ...
        - Column names:  
            -- `ADMIN1`: Name or abbreviation for the spatial subunit one level up from the nodes (e.g., Province, State, or Country)  
...
2. From the home directory of the country repository, create long-form geodata and mobility files with:
  `Rscript <path-to-COVIDScenarioPipeline>/COVIDScenarioPipeline/R/scripts/build_International_setup.R -c config.yml -p <path-to-spatial-repo> -w FALSE -n <name-of-population-data> -m <name-of-mobility-data>`
    
    This should look like this, since you should be in the home directory of your repository:
        `Rscript ./COVIDScenarioPipeline/R/scripts/build_International_setup.R -c config.yml -w FALSE -n population_data.csv -m mobility_data.csv`
