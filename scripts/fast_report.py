...
@click.option(
    ...
    default=500,
    help="Maximum number of sims to load",
)
@click.option(
    "-f",
    "--filename",
    type=str,
    default=f"{str(datetime.date.today())}_multiplot.pdf",
    help="filename to write the pdf in",
)
@click.option(
    "-c",
    "--config",
...
def generate_pdf(max_files, filename, config_file):
    ...
    for vi, var in enumerate(varplot):
        ...
        for scn, key in enumerate(list(all_hosp_sim.keys())):
            ...
            ax.plot(temp.quantile(0.5, axis=1), label=key, linewidth=2)
            ax.fill_between(
                temp.index,
                temp.quantile(0.05, axis=1),
                temp.quantile(0.95, axis=1),
                alpha=0.1,
            )
            ax.legend()
    ...
    for key, value in all_seir_sim_cumI.items():
        axes[0].plot(value, linewidth=0.3)
        axes[0].plot(value.quantile(0.5, axis=1), label=key, linewidth=2)
        axes[0].fill_between(
            value.index,
            value.quantile(0.05, axis=1),
            value.quantile(0.95, axis=1),
            alpha=0.1,
        )
    ...
    for key, value in all_seir_sim_diffI.items():
        axes[1].plot(value, linewidth=0.3)
        axes[1].plot(value.quantile(0.5, axis=1), label=key, linewidth=2)
        axes[1].fill_between(
            value.index,
            value.quantile(0.05, axis=1),
            value.quantile(0.95, axis=1),
