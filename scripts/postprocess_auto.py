...
def slack_multiple_files(token, message, fileList, channel):
    import logging
...
def generate_pdf(config_path, run_id, job_name, fs_results_path, slack_token, max_files, max_files_deep):
    ...
    for stat in gempyor.config["filtering"]["statistics"]:
        statistics[gempyor.config["filtering"]["statistics"][stat]["sim_var"].get()] = gempyor.config["filtering"]["statistics"][stat]["data_var"].get()
    statistics
