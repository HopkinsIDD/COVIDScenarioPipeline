...
title: "R Notebook"
params:
  config_file: config_2020_updated_incidC_incidH_shifts_alh.yml
...
llik_plot <- plot_llik_by_location(llik_interm_some,# %>% dplyr::filter(slot_num==1), #change to show for only a single slot
                                   filter_by="pdeath",
                                   filter_val=pdeath_default,
                                   burn_in = 0
)
...
cap<- paste0("**Fig. ", fig_counter, "** SEIR parameters by MCMC step ('chimeric' values)**")
...
snpi_plot <- plot_snpi_by_location(snpi_interm_some,#%>% dplyr::filter(slot_num==1),
                                   pdeath_filter = pdeath_default,
                                   scenario_filter = 'inference',
...
hnpi_plot <- plot_hnpi_by_location(hnpi_interm_some,#%>% dplyr::filter(slot_num==1),
                                   pdeath_filter = pdeath_default,
                                   scenario_filter = 'inference',
