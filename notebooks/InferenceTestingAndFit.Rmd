...
```{r epidraws2, echo=FALSE, fig.height=4, fig.width=6, message=FALSE, warning=FALSE, fig.cap="Draws of simulated epidemic trajectories (blue) and synthetic data on which calibration was done (black)."}
ggplot(long_sim_samples, aes(x = time, y = value)) +
  geom_line(aes(group = slot), alpha = .1, color = "blue") + 
  geom_point(data = obs_data, aes(x = date), size = .1, alpha = .5) +
...
```{r mse, fig.width=10, fig.height=5}
...
```{r acceptance2, fig.width=10, fig.height=5}
# Get acceptance IDs
sim_samples <- map_df(sim_files, function(x) read_csv(x, col_types = cols()) %>% 
                        mutate(slot = str_extract(x, "(?<=slot_)[0-9]+(?=_)"),
                               index = str_extract(x, "(?<=index_)[0-9]+(?=_m)"))) %>%
