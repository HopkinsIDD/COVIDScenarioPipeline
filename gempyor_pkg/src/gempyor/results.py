...
class Results:
    ...
    def plot_quick_summary(self, comp="cumI"):
        ...
        for i, nd in enumerate(np.arange(self.s.nnodes)):
            ax.plot(expand_data[nd][comp].quantile(0.5, axis=1))
            ax.fill_between(
                expand_data[nd][comp].index,
                expand_data[nd][comp].quantile(0.05, axis=1),
                expand_data[nd][comp].quantile(0.95, axis=1),
                alpha=0.3,
            )
        fig.autofmt_xdate()
        ...
        for i, nd in enumerate(np.arange(self.s.nnodes)):
            q50[i] = expand_data[nd][comp].quantile(0.5, axis=1)
            q95[i] = expand_data[nd][comp].quantile(0.95, axis=1)
            q05[i] = expand_data[nd][comp].quantile(0.05, axis=1)
    ...
    def save_output_for_R(self, seir):
        ...
        for s in range(self.s.nsim):
            ...
            out_df = pd.DataFrame(
                out_arr,
                columns=["comp"] + list(self.s.spatset.data["geoid"].astype(int)),
    ...
    def plot_comp(self, comp, nodes):
        ...
        for i, nd in enumerate(nodes):
            axes.plot(
                self.comp_data[nd][comp].quantile(0.5, axis=1),
                c=self.colors[i % 4],
            ...
            axes.fill_between(
                ...
                self.comp_data[nd][comp].quantile(0.95, axis=1),
                alpha=0.3,
