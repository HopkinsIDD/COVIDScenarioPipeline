...
option_list = list(
  ...
  optparse::make_option(c("-k", "--sims_per_slot"), action="store", default=Sys.getenv("COVID_SIMULATIONS_PER_SLOT", 1), type='integer', help = "Number of simulations to run per slot"),
  optparse::make_option(c("-n", "--slots"), action="store", default=Sys.getenv("COVID_NSIMULATIONS"), type='integer', help = "Number of slots to run."),
...
foreach(scenario = scenarios) %:%
  ...
  foreach(slot = seq_len(opt$slots),
    ...
    inference::filter_MC(
      ...
      jobs = 1,
      simulations_per_slot = opt$sims_per_slot,
      number_of_simulations = opt$slots,
      this_slot = slot,
      python = opt$python,
      rpath = opt$rpath,
