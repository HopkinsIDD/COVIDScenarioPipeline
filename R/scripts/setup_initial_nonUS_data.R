...
if (!is.null(config$outcomes$param_place_file)){
    ...
    hosp_death_ratio <- ifelse(is.null(config$outcomes$hosp_death_ratio), 7, config$outcomes$hosp_death_ratio)
    ...
    for(i in 1:length(ifr_labels)){
        ...
        arrow::write_parquet(rc, glue::glue("{config$spatial_setup$base_path}/hospitalization_inference_merge_{ifr_lab}.hpar.parquet"))
