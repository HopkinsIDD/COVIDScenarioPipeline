...
#####Caculate the first global likelihood based on initial conditions.
...
#####Create fist chimeric liklihoo dwith global_likelihood if it does not exist. Load from disk if needed
    if(!file.exists(first_chim_file)){
     ...
     rm(initial_sim_hosp) ###UNCLEAR WHY THIS IS HERE SPECIFICALLY
...
#####Get the full likelihood (WHY IS THIS A DATA FRAME)
    ...
    current_likelihood <- global_likelihood


    ## For logging
    current_index <- 0
    current_likelihood_data <- chimeric_likelihood_data
...
#####Loop over simulations in this block
   for( this_index in seq_len(opt$simulations_per_slot)) {
      ...
      arrow::write_parquet(chimeric_likelihood_data, this_llik_file)
