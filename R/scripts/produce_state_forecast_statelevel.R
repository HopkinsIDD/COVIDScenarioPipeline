...
opt$projection_date <- "2021-01-18"
opt$forecast_date <- "2021-01-03"
opt$end_date <- "2021-07-17"
...
if (opt$include_hosp) {
  ...
  state_weekly_incident_hosps <- res_state%>%
    mutate(week=lubridate::epiweek(time))%>%
    mutate(tmp_time = as.numeric(time))%>%
    group_by(USPS, sim_num, week)%>%
    summarize(incidH=sum(incidH),
              time=max(tmp_time))%>%
    ungroup() %>%
    mutate(time=lubridate::as_date(time))%>%
    group_by(time, USPS, week) %>%
  ...
  us_weekly_incident_hosps <- res_us %>%
    mutate(week=lubridate::epiweek(time))%>%
  ...
  if (opt$reichify)  {
    tmp_weekly_state_incident_hosp <-
      state_weekly_incident_hosps%>%
      mutate(forecast_date=opt$forecast_date)%>%
      rename(target_end_date=time)%>%
      mutate(location=as.character(cdlTools::fips(USPS)))%>%
      mutate(location=stringr::str_pad(location, width=2, side="left", pad="0"))%>%
      rename(value=incidH)%>%
      mutate(ahead=round(as.numeric(target_end_date - forecast_date)/7))%>%
      mutate(target=sprintf("%d wk ahead inc hosp", ahead)) %>%
      mutate(type="quantile")%>%
      mutate(type=replace(type, quantile=="mean","point"))%>%
      mutate(quantile=readr::parse_number(quantile)/100)%>%
      select(forecast_date, target, target_end_date,location,type, quantile, value)

    tmp_weekly_us_incident_hosp <-
      us_weekly_incident_hosps%>%
      mutate(forecast_date=opt$forecast_date)%>%
      rename(target_end_date=time)%>%
      mutate(location="US")%>%
      rename(value=incidH)%>%
    ...
    weekly_inc_hosps <-dplyr::bind_rows(tmp_weekly_state_incident_hosp, tmp_weekly_us_incident_hosp)
  }
}  
...
weekly_state_incident_cases <- res_state %>%
  ...
  mutate(time=lubridate::as_date(time))%>%
  group_by(time, USPS, week) %>%
  summarize(x=list(enframe(c(quantile(incidC, probs=c(0.01, 0.025,
                                                      seq(0.05, 0.95, by = 0.05), 0.975, 0.99)),
                             mean=mean(incidC)),
                           "quantile","incidC"))) %>%
  unnest(x)
