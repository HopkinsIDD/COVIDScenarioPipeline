...
print(paste0("Location: ", config$spatial_setup$setup_name))
...
## from JHU CSSE
# case_data <- covidImportation::get_clean_JHUCSSE_data(aggr_level = "UID", 
...
#                                    us_data_only=FALSE)
# 
# case_data <- case_data %>% dplyr::rename(date = Update,
#                                          geoid = FIPS)
# 
...
first_case <- incident_cases[1,]
first_case <- paste(paste(colnames(incident_cases), first_case, sep=": "), collapse = ", ")
print(paste0("First Seeded Cases: ", first_case))
...
print(paste0("Seeding saved to ", config$seeding$lambda_file))
