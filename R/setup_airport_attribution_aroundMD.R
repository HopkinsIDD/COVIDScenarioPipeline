...
shapefile_path <- paste0("data/around_md/", paste(states_of_interest, collapse = "_"), "-shp/", paste(states_of_interest, collapse = "_"), "-counties.shp")
...
big_airports_around_md <- read_csv("data/around_md/MD+VA+PA+NJ+DE+DC-airport_monthymeantravelers.csv") %>%
...
if(!file.exists("data/around_md/around_md_airports.csv")){
  ...
  write_csv(airports_to_consider, "data/around_md/around_md_airports.csv")
}

airports_to_consider <- read_csv("data/around_md/around_md_airports.csv") 
...
# Save it
write.csv(airport_attribution, file ='data/around_md/airport_attribution.csv', row.names=FALSE)
