...
output: 
  html_document: 
    ...
    fig_caption: true
params: 
  config_file: "config.yml" # add sanity: with run ("country" or "state"), country (e.g. "US"),  state (if run=="state"), and inference (T/F)
---
...
country <- config$sanity$country # US 
...
npi_names <- NA #must specify npi's used before projections
#npi_names <- names(config$interventions$settings)[which(str_detect(names(config$interventions$settings),fixed("intervention_inference", ignore_case = TRUE)))] 
...
jhu_data_dir = "JHU_CSSE_Data" #check
  ...
  # } else {
  #   ### Get updated version
  #   covidcommon:::update_JHUCSSE_github_data(jhu_data_dir)
  # }
...
```{r mdl_v_ob_death, fig.width=12, fig.cap=cap}
...
cap<-paste0("**Fig.", fig_counter, "**: Proportion of confirmed deaths outside the 95% prediction interval", if_else(run=="country", " by state", " by county "))
...
```{r mdl_v_ob_hosp, fig.width=12, fig.cap=cap}
...
cap<-paste0("**Fig.", fig_counter, "**: Estimated and confirmed cases over time by death rate.")
...
usa_sum%>%ggplot(aes(x=time, y=cum_case, ymin=cum_case_5pct, ymax=cum_case_95pct, linetype=death_Rate))+
  geom_line(col = "red ")+
...
res_state %>%
  ...
  group_by(scenario, death_Rate, group_mae)%>%
  ungroup() %>%
  ggplot(aes(x=time, y=cum_case, ymin=cum_case_5pct, ymax=cum_case_95pct))+
  geom_line(color = "red")+
  geom_ribbon(alpha=0.1, fill="red") +
  geom_point(aes(y=cumConfirmed), color="black")+
  facet_wrap(~death_Rate+group_mae, nrow=3) +
...
res_state %>%
  left_join(mae_infect, by = c(geogroup, "death_Rate", "scenario")) %>%
  {if(run=="state") unite(., "name_mae", Admin2:mae, sep=": ", remove = FALSE) else(unite(., "name_mae", !!as.symbol(geogroup):mae, sep=": ", remove = FALSE))} %>%
  inner_join(jhu_CSSE_dat%>%rename(time=date))%>%
