...
# Setting output/root directories by running rmarkdown::render() from terminal 
#Option 1: root.dir is set to the folder with the config, BUT only works if CSP is a subdirectory of COVID19_Location,
# and output.dir = getwd() in render()
...
```{r setup_config_vals}
# Config must have (alternate code could remove need for an additional config)
...
geo_names <- geo_names %>%
  mutate(name=factor(name, levels=sort(geo_names$name, decreasing=TRUE)),
         USPS=factor(USPS, levels=sort(unique(geo_names$USPS), decreasing=TRUE)))
...
inference_r <- inference_r %>%
  ...
  {if(affected_geoids) mutate(., npi_name=str_remove(npi_name, '^[[A-Z]].+\\_'))
    else(left_join(.))} %>%
  left_join(geo_names)
