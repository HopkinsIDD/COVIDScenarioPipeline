...
shp <- rgdal::readOGR(paste0(config$spatial_setup$base_path, "/", config$spatial_setup$setup_name,  "/", str_remove(config$spatial_setup$shapefile, "\\/.+$")), 
                      str_remove(str_remove(config$spatial_setup$shapefile, "^.+\\/"), ".shp"),
                      verbose=FALSE) # update to new shp file location 
...
if(run=="country" & country=="US"){
  included_geoids<-sort(c("AS", "GU", "MP", "PR", "VI", included_geoids), decreasing=TRUE)
}
...
`r paste0("**Tab.", tab_counter, "**: Summary of SEIR parameters")`
```{r param_summary}
...
hpar%>%
  {if(run=="state") filter(., geoid %in% included_geoids) else(.)} %>%
    ...
    flextable::flextable()%>%
    flextable::autofit()%>%
    flextable::set_header_labels(outcome="Outcome", death_rate="IFR", value="Probability", ci=paste0((pi_hi-pi_lo)*100, "% PI")) %>%
...
hpar%>%
  {if(run=="state") filter(., geoid %in% included_geoids) %>% left_join(., geo_names%>%select(geoid, name))
    else(left_join(., geo_names%>%select(USPS, geoid)))} %>%
  {if(run=="country") 
      mutate(., USPS=case_when(is.na(USPS) & geoid=="60000" ~ "AS",
                               is.na(USPS) & geoid=="66000" ~ "GU",
                               is.na(USPS) & geoid=="69000" ~ "MP",
                               is.na(USPS) & str_detect(geoid,"^72") ~ "PR",
                               is.na(USPS) & geoid=="78000" ~ "VI", 
                               TRUE~USPS)) 
    else(.)} %>%
...
snpi<- arrow::open_dataset(paste0(run_folder,"/snpi/"), 
                                   partitioning = c("location", "scenario", "death_rate", "date", "lik_type", "is_final", "sim_id")) %>%
   collect() %>%
   left_join(geo_names)%>%
  {if(run=="country") 
   ...
   group_by(sim_num, scenario, !!as.symbol(geogroup), npi_name, parameter, start_date, end_date)%>%
...
inference_r <- inference_r %>%
  ...
  summarize(r = mean(r_eff, na.rm=TRUE), 
            ...
            lower=quantile(r_eff, 0.025, na.rm=TRUE),
            upper=quantile(r_eff,0.975, na.rm=TRUE),
            reduc = mean(reduction, na.rm=TRUE), 
            reducq1 = quantile(reduction, 0.25, na.rm=TRUE), 
            reducq3 = quantile(reduction, 0.75, na.rm=TRUE)) %>%
  {if(run=="country") mutate(., npi_name=str_remove(npi_name, '^[[A-Z]]+\\_'),
                             USPS=factor(USPS, levels=sort(c(unique(geo_names$USPS), "AS", "GU", "MP", "VI"),decreasing=TRUE)))
    else(left_join(., geo_names) %>% mutate(., name=factor(name, levels=sort(unique(geo_names$name), decreasing=TRUE))))} 
...
truth_dat <- get_USAFacts_data() %>%
  filter({if(run=="state") FIPS else(source)} %in% included_geoids) %>%
  rename(date=Update, geoid=FIPS, USPS=source) %>%
...
if(run=="state"){
  ...
  if(hosp_eval){#comparison of hospital occupancy is currently only for "state", will require manual setup
...
for(i in 1:length(pdeath_levels)){
  res_state[[i]] <- arrow::open_dataset(paste0(run_folder,"/hosp/"),
    ...
    filter(death_rate==pdeath_levels[i]) %>%
  ...
  if(i==length(pdeath_levels)){
    res_state<- bind_rows(res_state)%>%
      mutate(death_rate=factor(death_rate, levels=pdeath_levels))%>%
      {if(run=="state") left_join(., geo_names) %>% mutate(., name=factor(name, levels=sort(unique(geo_names$name), decreasing=TRUE)))
        else(mutate(., USPS=factor(USPS, levels=sort(c(unique(geo_names$USPS), "AS", "GU", "MP", "VI"), decreasing=TRUE))))}
  }
...
if(country=="US" & run=="country"){
  usmap::plot_usmap(data=mapdat, values="death_ratio")+
...
if(country=="US" & run=="country"){
  mapdat%>%
    ...
    usmap::plot_usmap(data=., values="value")+
