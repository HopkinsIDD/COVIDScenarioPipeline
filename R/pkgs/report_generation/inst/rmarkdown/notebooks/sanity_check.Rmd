...
output: 
  html_document: 
    ...
    fig_caption: true
params: 
  config_file: config_inference_territories_high_report.yml # add sanity: with run ("country" or "state"), country (e.g. "US"),  state (if run=="state"), and inference (T/F)
...
knitr::opts_knit$set(eval.after = 'fig.cap', 
                     root.dir = "/Users/juandenthulse/Documents/GitHub/COVID19_USA") # assumes in notebook subdirectory
...
fit_date<-if_else(is.null(config$sanity$fit_date), Sys.Date(), lubridate::ymd(config$sanity$fit_date))
...
inference_r <- inference_r %>%
  ...
  # {if(run=="country") mutate(., npi_name=str_remove(npi_name, '^[[A-Z]]+\\_'),
  #                            USPS=factor(USPS, levels=sort(c(unique(geo_names$USPS), "AS", "GU", "MP", "PR", "VI"),decreasing=TRUE)))
  ...
  if(i==length(pdeath_levels)){
    res_state<- bind_rows(res_state)%>%
      ...
      {if(run=="state") left_join(., geo_names) %>% mutate(., name=factor(name, levels=sort(unique(geo_names$name), decreasing=TRUE)))
        else(mutate(., USPS=factor(USPS, levels=sort(c(unique(geo_names$USPS), "AS", "GU", "MP","PR", "VI"), decreasing=TRUE))))}
  }
