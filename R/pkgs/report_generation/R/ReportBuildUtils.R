...
plot_ts_hosp_state_sample <- function (hosp_state_totals, 
  ...
  to_plt <- hosp_state_totals %>% 
    ...
    dplyr::filter(sim_num %in% sample(unique(sim_num), min(num_sims, length(unique(sim_num))), replace=FALSE)) %>%
  ...
  rc <- ggplot(data=to_plt, aes(x=time, colour = scenario_name, group = interaction(sim_num, scenario_name))) +
    geom_line(aes(y = Nhosp), alpha=0.3, size=.75) +
    scale_y_continuous("Daily hospital occupancy", labels = scales::comma) +
    scale_x_date(date_breaks = "1 month", date_labels = "%b", limits = c(as.Date(sim_start_date), as.Date(sim_end_date))) +
  ...
  if(plot_intervention){
    interv_dates <- data.frame(xmin = as.Date(interv_start_date), xmax = as.Date(interv_end_date), ymin = 0, ymax = max(to_plt$Nhosp) + 0.05*max(to_plt$Nhosp))

    rc <- rc +
      geom_rect(data = interv_dates, inherit.aes = FALSE,
                aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax), color = "grey", fill = "grey", alpha = 0.33) 
  }
...
plot_ts_incid_inf_state_sample <- function (hosp_state_totals, 
  ...
  rc <- ggplot(data=to_plt, aes(x = time, color = scenario_name, group = interaction(sim_num, scenario_name))) +
  ...
  if(plot_intervention){
    interv_dates <- data.frame(xmin = as.Date(interv_start_date), xmax = as.Date(interv_end_date), ymin = 0, ymax = max(to_plt$NincidInf) + 0.05*max(to_plt$NincidInf))
...
plot_ts_incid_death_state_sample_allPdeath <- function (hosp_state_totals,
  ...
  to_plt <- hosp_state_totals %>% 
    dplyr::filter(sim_num %in% sample(unique(sim_num), min(num_sims, length(unique(sim_num))), replace=FALSE)) %>%
    dplyr::mutate(scenario_name = factor(scenario_name, levels = params$scenario_labels)) %>%
    dplyr::mutate(pdeath = factor(pdeath, levels = params$pdeath_filecode, labels = params$pdeath_labels)) %>%
    dplyr::mutate(sim_num = factor(sim_num))
  
  rc <- ggplot(data=to_plt, aes(x=time, color=scenario_name, group=interaction(sim_num, scenario_name))) +
    geom_line(aes(y=Ndeath), alpha=0.2, size=.75) +
    scale_y_continuous("Daily incident deaths", labels = scales::comma) +
    scale_x_date(date_breaks = "2 months", date_labels = "%b", limits = c(as.Date(sim_start_date), as.Date(sim_end_date))) +
    scale_color_manual("Scenario",
                      labels = params$scenario_labels,
                      values = params$scenario_cols) +
    theme_minimal() +
  ...
  if(plot_intervention){
    interv_dates <- data.frame(xmin = as.Date(interv_start_date), xmax = as.Date(interv_end_date), ymin = 0, ymax = max(to_plt$Ndeath) + 0.05*max(to_plt$Ndeath))
