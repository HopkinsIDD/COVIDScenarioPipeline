...
nfiles <-  10
pdeath_levels <- c("high", "med", "low") #config$hospitalization$parameters$p_death_names
...
npi_names <- names(config$interventions$settings)[2:3] #must specify (npi's used before our scenarios, for CA these were "SchoolClosurePartial" and "LockdownPartial")
npi_label <- c("School Lockdown (March 13-19)", "Lockdown (March 19 - May 8)") #can specify
...
plot %>%
    left_join(label) %>%
    bind_rows(county_r0) %>%
    mutate(npi_label = factor(npi_label, levels = c("Baseline", label$npi_label)),
...
plot %>%
    ...
    mutate(name = factor(name, levels = sort(geo_names$name,decreasing=TRUE)),
           npi_label = factor(npi_label, levels = c(label$npi_label))) %>%
...
`r paste0("**Fig.", fig_counter, "**: Modeled deaths vs. latest reported deaths by death rate with a 14-day delay.")`
...
`r paste0("**Fig.", fig_counter, "**: Modeled vs latest reported deaths by death rate and ", if_else(run=="country", "state.", "county."), "with a 14-day delay.")`
...
`r paste0("**Fig.", fig_counter, "**: Ratio of modeled deaths to latest reported deaths", if_else(run=="country", " by state", " by county"), "with a 14-day delay.")`
