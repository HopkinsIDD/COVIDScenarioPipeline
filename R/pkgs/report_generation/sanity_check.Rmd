...
knitr::opts_knit$set(root.dir = "/Users/juandenthulse/Documents/GitHub/COVID19_California") # assumes in notebook subdirectory
...
nfiles <-  NA 
pdeath_levels <- c("high", "med", "low") #ccan also specify with config$hospitalization$parameters$p_death_names
...
```{r param_summary, fig.cap=cap}

cap<-paste0("**Tab.", tab_counter, "**: Summary of hospitalization and SEIR parameters")
...
````{r scen_summary, eval = {if(run=="state") TRUE else(FALSE)}, fig.cap=cap}
...
cap<-paste0("**Fig.", tab_counter, "**: Estimated reproducive number at baseline and during previous interventions")
...
plot %>%
    ...
    mutate(npi_label = factor(npi_label, levels = c("Baseline", unique(label$npi_label))),
           name = factor(name, levels = sort(geo_names$name, decreasing=TRUE))) %>%
...
cap<-paste0("**Fig.", tab_counter, "**: Estimated effectiveness of previous interventions")

plot %>%
    left_join(label) %>%
    mutate(name = factor(name, levels = sort(geo_names$name,decreasing=TRUE)),
           npi_label = factor(npi_label, levels = c(unique(label$npi_label)))) %>%
...
cap<-paste0("**Tab.", tab_counter, "**: Model fit metrics versus JHU data")
...
cap<-paste0("**Fig.", fig_counter, "**: Modeled infections vs confirmed cases over time by death rate. Assumes 1:5 ratio of modeled hospitalizations to infections.")
...
cap<-paste0("**Fig.", fig_counter, "**: Mean absolute error comparing modeled infections and confirmed cases in", {if(run=="state") " counties" else(" states")}," by death rate")
...
cap<-paste0("**Fig.", fig_counter, "**: Relative mean absolute error comparing modeled infections (assuming 1:5 hospitalization ratio) and confirmed cases", if_else(run=="country", " by state", " by county "))
...
cap<-paste0("**Fig.", fig_counter, "**: Proportion of confirmed infections within 95% prediction interval", {if(run=="state") " counties" else(" states")}," by death rate")
...
cap<-paste0("**Fig.", fig_counter, "**: Modeled infections vs confirmed cases over time in the four", {if(run=="state") " counties" else(" states")}," with the largest MAE by death rate")
...
```{r, fig.height=70, fig.cap=cap}
cap<-paste0("**Fig.", fig_counter, "**: Estimated vs reported  infections over time across", {if(run=="state") " counties " else(" states ")} ,"by death rate.  Assumes 1:5 ratio of modeled hospitalizations to infections.")
