...
library(tidyverse)
#knitr::opts_knit$set(root.dir = "d:/COVID19_California")
...
#key_group <- unlist(config$sanity$key_group, use.names=FALSE)
...
`r paste0("**Fig.", fig_counter, "**: Modeled deaths vs. latest reported deaths by death rate. Modeled deaths have a ", t_report, "-day lag.")`
...
```
`r paste0("**Fig.", fig_counter, "**: Modeled vs latest reported deaths by death rate and ", if_else(run=="country", "state.", "county."), " Modeled deaths have a ", t_report, "-day lag.")`
...
`r paste0("**Fig.", fig_counter, "**: Ratio of modeled deaths to latest reported deaths", if_else(run=="country", " by state", " by county"), " with low death rate.", " Modeled deaths have a ", t_report, "-day lag.")`
...
shp <- rgdal::readOGR(paste0(config$spatial_setup$base_path, "/", str_remove(config$spatial_setup$shapefile, "\\/.+$")),
...
`r paste0("**Fig.", fig_counter, "**: Modeled vs reported deaths over time. Modeled deaths have a ", t_report, "-day lag.")`  
...
`r paste0("**Fig.", fig_counter, "**: Mean average error for deaths (with ", t_report, " day lag)", if_else(run=="country", " by state", " by county "))`  
...
mae <- res_state%>%
  ...
  distinct() %>%
  mutate(lead_death = lead(cum_death, t_report),
         error = abs(lead_death-cumDeaths)) %>%
...
`r paste0("**Fig.", fig_counter, "**: Modeled vs reported deaths over time in the four", {if(run=="state") " counties" else(" states")} ," with the largest MAE by death rate.", "Modeled deaths have a ", t_report, "-day lag.")`  
...
`r paste0("**Fig.", fig_counter, "**: Mean average error comparing observed and modelled deaths with a ", t_report, " lag")`
...
deaths <- res_state %>% group_by(scenario, death_Rate, !!as.symbol(geogroup)) %>%
  ...
  summarize(cumDeaths = sum(cumDeaths),
            lead_death_5pct = sum(lead_death_5pct),
            lead_death_95pct = sum(lead_death_95pct),
            lead_death = sum(lead_death)) %>%
  #group_by(death_Rate) %>%
  filter(!is.na(lead_death)) %>%
  mutate(error = abs(lead_death-cumDeaths),
         within = if_else(cumDeaths>lead_death_95pct |
                            cumDeaths<lead_death_5pct, 0, 1)) %>%
...
res_state %>% group_by(scenario, death_Rate, !!as.symbol(geogroup)) %>%
  ...
  summarize(confirmed = sum(confirmed),
            adjI_5pct = sum(adjI_5pct),
            adjI_95pct = sum(adjI_95pct),
            adjI = sum(adjI)) %>%
  #group_by(death_Rate) %>%
  filter(!is.na(adjI)) %>%
  mutate(error = abs(adjI-confirmed),
