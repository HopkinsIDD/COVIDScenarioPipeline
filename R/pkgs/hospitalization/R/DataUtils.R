...
## NOTES from jwills on port
## This was original derived from the DataUtils_withoutICUcap.R file
## with modifications made to also be able to support the logic from
## the DataUtils_withHospCapacity.R file (which was highly similar, but
## not indentical, to the DataUtils_withoutICUpcap.R file.) In the code
## below, searching for the string "jwills" will find all of the places
## where I modified the functionality to unify the different logic across
## the two files so that a single package could support report generation
## from Rmarkdown files that used _either_ of these two files.
...
_get_hosp_table_report_vars <- function(include_icu_pcap = FALSE) {
    ...
    if (include_icu_pcap) {
      var_to_report <- c(var_to_report, "peak_ICU_cap_")
    }
    var_to_report <- c(var_to_report, "vent_est_", "death_est_")
    return(var_to_report)
}
...
##'@param include_icu_pcap whether to include the icu_pcap in the returned data table
##'
make_hosp_table <- function(final_hosp_dat, final_hosp_metrop_dat, p_death, include_icu_pcap=FALSE){
...
##'@param p_death IFRs used in this analysis
##'@param include_icu_pcap Whether to include icu_pcap variables in the returned analysis
##'
make_hosp_table_county <- function(final_hosp_dat, final_hosp_geoid_dat, county_dat, p_death, include_icu_pcap = FALSE){
    ...
    tmp_geoid <- final_hosp_geoid_dat %>%
        mutate(hosp_est = paste0(conv_round(nhosp_final), " (", conv_round(nhosp_lo), "-", conv_round(nhosp_hi), ")"),
               ...
               death_est = paste0(conv_round(ndeath_final), " (", conv_round(ndeath_lo), "-", conv_round(ndeath_hi), ")")) %>%
        arrange(desc(nhosp_final)) %>%
        select(geoid_label, p_death, hosp_est, peak_hosp, peak_hosp_cap, ICU_est, peak_ICU, peak_ICU_cap, vent_est, death_est) %>%
...
##'
make_final_map <- function(ca_final_dat, title="", legend, scale=NULL){

    #NOTE(jwills): logic to merge withoutICUCap and withHospCapacity versions of this code here
    limits <- NULL
    add_labs <- FALSE
    if (is.null(scale)) {
      scale <- "plasma"
      ...
      ca_final_dat$fill <- ca_final_dat$ar
      add_labs <- TRUE
    }
    ...
    p <- ggplot(ca_final_dat) +
        ...
        theme_minimal() +
        scale_fill_viridis(option=scale, limits=limits) +
        ggtitle(title) +
...
##'@param include_icu_pcap whether to include ICU pcap data
##'
##'@return df object of estimates + CIs for hosp/deaths at all p_death
##'
make_hospdeath_table1 <- function(sim_hospdeath_dat, include_icu_pcap=FALSE){
    tmp <- sim_hospdeath_dat$res_total %>%
        ...
        bind_rows(sim_hospdeath_dat$res_total %>%
                      mutate(ci = make_CI(nhosp_curr_lo, nhosp_curr_hi),
                             est = conv_round(nhosp_curr_final),
        ...
        tmp <- tmp %>%
            bind_rows(sim_hospdeath_dat$res_total %>%
                          mutate(ci = make_CI(nicu_curr_lo, nicu_curr_hi),
                                 est = conv_round(nicu_curr_final),
