...
test_that("MCMC step copies are correctly performed when we are not at the start of a block", {
    ...
    res <- perform_MCMC_step_copies(current_index,
                                    slot,
                                    block,
                                    run_id,
                                    global_local_prefix,
...
test_that("MCMC step copies are correctly performed when we are at the start of a block", {
    ##some information on our phantom runs
    current_index <- 0
    slot <- 2
    block <- 5
