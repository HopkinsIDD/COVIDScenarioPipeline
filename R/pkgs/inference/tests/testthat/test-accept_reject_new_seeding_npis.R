...
test_that("all blocks are accpeted when all proposals are better",{
    ...
    tmp <- accept_reject_new_seeding_npis(seed_orig, seed_prop, npis_orig, npis_prop, orig_lls, prop_lls)



    expect_that(tmp$seeding$value, equals(seed_prop$value))
    expect_that(tmp$npis$value, equals(npis_prop$value))
...
test_that("all blocks are rejected when all proposals are 1x10^12 times worse",{
    ...
    expect_that(tmp$npis$value, equals(npis_orig$value))
...
test_that("only middle block is accepted when appropriate",{
    ...
    expect_that(tmp$npis$value[npi_inds], equals(npis_orig$value[npi_inds]))
    ...
    expect_that(tmp$npis$value[npi_inds], equals(npis_prop$value[npi_inds]))
