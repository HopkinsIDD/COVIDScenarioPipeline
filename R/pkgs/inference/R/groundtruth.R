...
get_ground_truth_file <- function(data_path, cache = TRUE, 
    ...
    if(!(file.exists(data_path) & cache)){
        ...
        cases_deaths <- suppressMessages(
            covidcommon::get_groundtruth_from_source(source = gt_source, scale = gt_scale, source_file = gt_source_file,
                                                     variables = targets,
                                                     incl_unass = ifelse(gt_scale == "US state", TRUE, FALSE), 
                                                     fix_negatives = fix_negatives,
                                                     adjust_for_variant = !is.null(variant_filename), 
                                                     variant_props_file = variant_filename))
