...
aggregate_and_calc_loc_likelihoods <- function(all_locations,
  ...
  for (prior in names(defined_priors)) {
    ...
    # Add prior contribution computed over all locations for which the likelihood is 
    # not already computed
    ll_adjs <- ll_adjs %>%
      filter(!(geoid %in% unique(likelihood_data$geoid))) %>% 
      summarise(likadj = sum(likadj),
                geoid = "all") %>% 
      rbind(ll_adjs)
    ...
    ##probably a more efficient what to do this, but unclear...
    likelihood_data <- dplyr::left_join(likelihood_data, ll_adjs) %>%
      dplyr::mutate(ll = ll + ifelse(is.na(ll), 0, ll)) %>%
      dplyr::select(-likadj)
  }
...
##'@param opt the options passed to filter_MC.R
