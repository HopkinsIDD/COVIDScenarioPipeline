...
params:
  config_file: config_inference_ca_high_report.yml
...
death_rate_levels<-c("high", "med", "low")
...
cum_inf <- arrow::open_dataset(paste0(run_folder,"/hosp/"), 
                               partitioning = c("location", "scenario", "death_rate", "date", "lik_type", "is_final", "sim_id")) %>%
  collect() %>%
  group_by(geoid, death_rate, scenario, sim_id) %>%
  select(incidI, time)%>%
  mutate(cum_infect=cumsum(incidI),
         time=as.Date(time)) %>%
  filter(time>lubridate::ymd("2020/03/20"), time<lubridate::ymd("2020/07/31")) %>%
  group_by(geoid, scenario, time) %>%
...
res_state<- arrow::open_dataset(paste0(run_folder,"/hosp/"), 
  ...
  group_by(geoid, death_rate, scenario, sim_id, time) %>%
...
res_state <- res_state%>%
  ...
  mutate(scenario_name="inference_only") %>%
  left_join(geo_names) %>%
  mutate(name = factor(name, levels=sort(geo_names$name, decreasing=TRUE)))
...
covid <- truth_dat %>%
  group_by(geoid, name, week=lubridate::floor_date(date, unit="week", week_start=6)) %>%
  summarize(incidI = sum(incidI))%>%
  mutate(Confirmed=cumsum(incidI))
...
plot_r_comp <- function(r0_dat=rt, 
                        ...
                        end_date="2020-06-05" # for R estimates from r0_dat (ymd format e.g. "2020-05-30")
                        ){
...
r_comp <- r_comp %>%
  ...
  left_join(
    covid_dat%>%
      ...
      group_by(name)%>%
      summarize(`Confirmed cases`=max(Confirmed))) %>%
...
spread<-r_comp %>%
  ungroup() %>%
  summarize(q1=quantile(ratio, 0.25), 
            q3=quantile(ratio, 0.75),
            iqr=IQR(ratio))

r_comp <- r_comp%>%
  mutate(outlier=case_when(ratio < spread$q1 - 1.5*spread$iqr ~ 1, 
                           ratio > spread$q3 + 1.5*spread$iqr ~ 1,
                           TRUE ~ 0))
...
for(i in 1:length(npiselection)){
  plot_r_comp[[i]] <- r_comp %>%
    ...
    geom_abline() +
    ggrepel::geom_text_repel(aes(label=if_else(outlier==1, as.character(name),
                                               '')), segment.size = 0.2, alpha = 0.55)+
    theme_bw() +
    theme(panel.grid.minor = element_blank(),
          legend.position = "bottom") +
    xlim(c(0,3))+
...
```{r r0_pack2, eval=FALSE}  

# # Ignore for now - considering plotting Rt model estimates for forecasted period
# 
# covid <- truth_dat %>%
...
# 
# rt%>%
#   filter(week>lubridate::ymd("2020-03-01") & week<max(week))%>%
#   ggplot(aes(x=week))+
#   geom_line(aes(y=Rt), col="red")+
#   geom_ribbon(aes(ymin=lower, ymax=upper), fill = "red", alpha=0.1)+
#   facet_wrap(~name, scales="free_y")+
#   geom_hline(yintercept=1)

covid_model<-res_state%>%
  group_by(death_rate, week=lubridate::floor_date(time, unit="week", week_start=2), sim_id, geoid)%>%
  summarize(NincidInf=sum(NincidInf), 
            NincidCase=sum(NincidCase),
            cum_inf=sum(cum_inf),
            cum_case=sum(cum_case)) %>%
  group_by(death_rate, week, geoid)%>%
  summarize(NincidInf=round(mean(NincidInf)), 
            NincidCase=round(mean(NincidCase)),
            cum_inf=round(mean(cum_inf)),
            cum_case=round(mean(cum_case))) %>%
  group_by(death_rate, geoid)%>%
  filter(cum_inf!=0,
         NincidInf!=0) %>%
  left_join(shp%>%sf::st_drop_geometry()%>%select(name,geoid))%>%
  left_join(geodata%>%select(geoid, pop2010))
  
counties <- covid_model%>%
  ungroup()%>%
  distinct(name)%>%
  arrange(name)%>%
  unlist(use.names=FALSE)
...
for(i in 1:length(counties)){
  temp<-list()
  for(j in 1:length(death_rate_levels)){
    county <- covid_model %>%
        filter(name==counties[i],
               death_rate==death_rate_levels[j])
    
    incid <- setNames(county$NincidInf, 1:nrow(county))
    pop <- county$pop2010[i]
    mGT <- R0::generation.time("gamma", c(6.5/7, 4.5/7))
    estR0 <- R0::estimate.R(incid, mGT, begin=1, end=as.numeric(length(incid)), methods=c("TD"), pop.size = pop, nsim=1000)
    Rt1 <- tibble(week = county$week, Rt = estR0$estimates$TD$R, estR0$estimates$TD$conf.int)
    temp[[j]] <- Rt1 %>% 
      mutate(name = counties[i],
             death_rate=death_rate_levels[j]) %>%
...
bind_rows(rt)%>%
  filter(week>=lubridate::ymd("2020-06-09"), min(week)<week)%>%
  filter(death_rate=="high")%>%
  ggplot(aes(x=week))+
  geom_line(aes(y=Rt), col="red")+
  geom_ribbon(aes(ymin=lower, ymax=upper), fill = "red", alpha=0.1)+
  facet_wrap(~name, scales="free_y")+
...
state_hosp_totals<- arrow::open_dataset(paste0(run_folder,"/hosp/"), 
                               partitioning = c("location", "scenario", "death_rate", "date", "lik_type", "is_final", "sim_id")) %>%
  collect() %>%
  group_by(geoid, death_rate, scenario, sim_id) %>%
  mutate(cum_hosp=cumsum(incidH)) %>%
  ...
  mutate(cum_case=cumsum(incidC)) %>%
  mutate(cum_inf=cumsum(incidI)) %>%
  group_by(death_rate, scenario, sim_id, time) %>%
  summarize(NhospCurr=sum(hosp_curr),
            NICUCurr=sum(icu_curr), 
            NincidDeath=sum(incidD), 
            NincidInf=sum(incidI), 
...
state_hosp_totals <- state_hosp_totals%>%
  ...
  group_by(death_rate, scenario) %>%
  mutate(sim_num=seq_along(sim_id)) %>%
  right_join(state_hosp_totals) %>%
  mutate(scenario_name="inference_only") # modify as needed
...
bed_capacity<-bed_capacity%>%
  ...
  summarize(tot_avail=round(mean(avail_med_surgical_beds, na.rm=TRUE))+
            ...
            # tot_avail=round(mean(avail_med_surgical_beds, na.rm=TRUE))+
            #   round(mean(total_conf_hosp_patients)),
            # tot_icu_avail=round(mean(avail_icu_beds, na.rm=TRUE))+
            #   round(mean(total_conf_icu_patients)),
            tot_icu_avail=round(mean(avail_icu_beds, na.rm=TRUE))+
  ...
  mutate(tot_avail=if_else(is.na(tot_avail), 0, tot_avail),
         ...
         # tot_avail=if_else(is.na(tot_avail), 0, tot_avail),
         # tot_icu_avail=if_else(is.na(tot_icu_avail),0, tot_icu_avail),
