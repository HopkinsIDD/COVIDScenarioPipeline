...
projection_start <- ifelse(params$projection_start=="YYYY/MM/DD", Sys.Date(), lubridate::ymd(params$projection_start))
...
res_state<- load_hosp_sims_filtered(runs_dir)
...
truth_dat <- load_USAFacts_for_report(states=modeled_states) %>%
...
truth_dat%>%
  ...
  plot_rt_ts(outcome_dir=runs_dir,
             truth_dat=.,
             susceptible=TRUE,
...
plot_inference_r(r_dat=inference_r, 
                 ...
                 npi_label=NA, # or npi_label[-1] if not NA
                 effectiveness=TRUE,
...
`r paste0("**Tab. ", tab_counter, "** Summary estimates across counties for the ",format(projection_start,"%B %d"),"-", format(sim_end_date,"%B %d"), " time period with ",pdeath_default_label," assumptions.")`  
```{r county_tab}
...
cap <- paste0("**Fig. ", fig_counter, "** Population-adjusted deaths, hospitalizations, and confirmed cases between ", format(projection_start, "%B %d"), "-", format(short_end, "%B %d"), ". Estimates by county and 1% IFR (red), 0.5% IFR (green), and 0.25% IFR (blue).")  
...
cap<- paste0("**Fig. ", fig_counter, "** Population-adjusted estimates of hospitalizations between ", format(projection_start, "%B %d"), "-", format(short_end, "%B %d"), " assuming the effectiveness of post-lockdown social distancing is maintained by county. Estimates shown for 1% IFR (red), 0.5% IFR (green), and 0.25% IFR (blue) assumptions. ")  
...
`r paste0("**Tab. ", tab_counter, "** Summary estimates across counties for the ", format(projection_start, "%B %d"), "-", format(sim_end_date, "%B %d"), " time period for all IFR assumptions.")`  
```{r county_tab_all}
...
cap <- paste0("**Fig. ", fig_counter ,"** Estimated number of incident cases (left) and occuppied hospital (center) and ICU beds (right) by county between ", format(projection_start, "%B %d")," and ", format(sim_end_date, "%B %d"),". Mean estimates are shown at 1% IFR (red), 0.5% IFR (green), and 0.25% IFR (blue).")
