...
```{r load_case_data}
## load case data 
usaf_dat <- load_USAFacts_for_report(states = modeled_states) %>%
...
```{r, include=FALSE}
p1<-plot_scn_outcomes_ratio(hosp_state_totals = state_hosp_totals,
                               start_date=first(display_dates),
                               end_date=last(display_dates),
                               deathRate=pdeath_default,
                               scenario_labels=scn_labels,
                               scenario_levels=scn_levels,
...
p2<-plot_scn_outcomes_ratio(hosp_state_totals = state_hosp_totals,
                               start_date=display_dates[1],
                               end_date=display_dates[2],
                               deathRate=pdeath_default,
                               scenario_labels=scn_labels,
                               scenario_levels=scn_levels,
                               scenario_cols=scn_colors, 
...
p3<-plot_scn_outcomes_ratio(hosp_state_totals = state_hosp_totals,
                               start_date=display_dates[2],
                               end_date=display_dates[3],
                               deathRate=pdeath_default,
                               scenario_labels=scn_labels,
                               scenario_levels=scn_levels,
                               scenario_cols=scn_colors, 
...
p4<-plot_scn_outcomes_ratio(hosp_state_totals = state_hosp_totals,
                               start_date=display_dates[3],
                               end_date=display_dates[4],
                               deathRate=pdeath_default,
                               scenario_labels=scn_labels,
                               scenario_levels=scn_levels,
                               scenario_cols=scn_colors, 
...
p1<-p1+theme(legend.position="none",
    ...
    labs(subtitle=paste0(format(first(display_dates), "%B %d"), 
                         "-",
                         format(last(display_dates), "%B %d")))
...
p2<-p2+theme(legend.position="none",
                ...
                plot.subtitle = element_text(size=18,
                                             face="bold"))+
    labs(subtitle=paste0(format(display_dates[1], "%B %d"), 
                         "-",
                         format(display_dates[2], "%B %d")))
...
p3<-p3+theme(legend.position="none",
                ...
                plot.subtitle = element_text(size=16,
                                             face="bold"))+
    labs(subtitle=paste0(format(display_dates[2], "%B %d"), 
                         "-",
                         format(display_dates[3], "%B %d")))
...
p4<-p4+theme(legend.position="none",
                ...
                plot.subtitle = element_text(size=18,
                                             face="bold"))+
    labs(subtitle=paste0(format(display_dates[3], "%B %d"), 
                         "-",
                         format(display_dates[4], "%B %d")))

plot_time<-ggarrange(plots=list(p1, p2, p3, p4),
                     nrow=1,
                     bottom=grid::textGrob('Relative to "Business as usual" scenario', gp = grid::gpar(col = "black", fontsize = 16)))
...
cap<-paste0("**Fig. ", fig_counter, '** The mean ratio of summarized outcomes in each scenario relative to the "Business as usual" scenario with 95% prediction intervals. Estimates are shown across 4 time periods at ',pdeath_default_label,' assumptions.')
...
for (i in 1:length(display_dates)) {
    hist_plts[[i]] <- plot_hist_incidHosp_state(state_hosp_totals,
                                               ...
                                               scenario_levels = scn_levels,
                                               scenario_labels =  scn_labels,
                                               scenario_cols =  c(scn_colors, "dodgerblue3"),
                                               sim_start_date = sim_start_date,
...
```{r scn_state_table1}
make_scn_state_table_withVent(scn_labels[1],
                              state_hosp_totals,
