...
projection_start <- ifelse(params$projection_start=="YYYY/MM/DD", lubridate::ymd(Sys.Date()), lubridate::ymd(params$projection_start))
...
usaf_dat <- load_USAFacts_for_report(states = modeled_states) %>%
  ...
  filter(date<as.Date(projection_start)) %>%
  left_join(geodata)
```
